/*
  ...
  1. New Tables
    ...
    - `players` - Player data including assignments and stats  
    - `elimination_claims` - Claims for player eliminations with witness system
    - `chat_messages` - In-game chat messages
    - `leaderboard` - Global player statistics and rankings

  2. Security
    - Enable RLS on all tables
    - Add policies for public access (suitable for game environment)
    - Create indexes for optimal performance

  3. Features
    - Custom enum types for game states and claim responses
    - Automatic leaderboard updates via triggers
